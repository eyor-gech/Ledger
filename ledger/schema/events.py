...
Canonical domain event catalogue (Phase 0–2).

Key constraints:
- Pydantic v2 models for all domain event payloads (type-safe, strict).
- BaseEvent (domain) and StoredEvent (persisted) are distinct types.
...
class FinancialFacts(LedgerModel):
    # Income statement
...
class FraudAnomaly(LedgerModel):
...
class CreditDecision(LedgerModel):
    risk_tier: RiskTier
...
class BaseEvent(LedgerModel):
    """Domain event (payload + type + version). Store-assigned fields live in StoredEvent."""

    event_type: str
    event_version: int = 1

    # Optional for command-side idempotency; EventStore always assigns an event_id.
...
class StoredEvent(LedgerModel):
    """Persisted event as returned by the EventStore."""
    ...
    def from_row(cls, row: dict[str, Any]) -> "StoredEvent":
        ...
        return cls(
            ...
            prev_hash=row.get("prev_hash").hex() if isinstance(row.get("prev_hash"), (bytes, bytearray)) else (row.get("prev_hash") or None),
            event_hash=row.get("event_hash").hex() if isinstance(row.get("event_hash"), (bytes, bytearray)) else (row.get("event_hash") or None),
        )


class StreamMetadata(LedgerModel):
    stream_id: str
    aggregate_type: str
    current_version: int
    created_at: datetime
...
class AgentContextLoaded(BaseEvent):
    """Context snapshot marker to support Gas Town recovery."""
