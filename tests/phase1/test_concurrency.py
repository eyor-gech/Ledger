...
async def test_many_concurrent_appends_one_winner():
    ...
    await store.append("s", [_ev("Init")], expected_version=-1)
...
async def test_two_truly_concurrent_appends_one_failure_has_structured_occ():
    ...
    results = await asyncio.gather(
        store.append("s", [_ev("A")], expected_version=0),
        store.append("s", [_ev("B")], expected_version=0),
...
async def test_per_stream_ordering_is_sequential():
    ...
    for i in range(50):
        ...
        await store.append("stream-a", [_ev("E", i=i)], expected_version=ver)
...
async def test_global_positions_monotonic_across_streams():
    store = InMemoryEventStore()
    await store.append("a", [_ev("E1"), _ev("E2")], expected_version=-1)
    await store.append("b", [_ev("E3")], expected_version=-1)
    await store.append("a", [_ev("E4")], expected_version=1)
